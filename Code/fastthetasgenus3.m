...
function NaiveThetaConstantsGenus3(tau, check, Bbasic)
  ...
  if Bbasic eq 0 then
  	c := Min(  Min(Imaginary(tau[1][1]-tau[1][2])-Abs(Imaginary(tau[1][3])), Imaginary(tau[2][2]-tau[1][2]-tau[2][3]) ),   Imaginary(tau[3][3]-tau[2][3])-Abs(Imaginary(tau[1][3]))     );
  	d := 1/100;      // cf Marco's lemma in the paper: n Y ^tn >= d Im(tau_{11}) (n_1^2+n_2^2+n_3^2)
  	c := Max(c, d*Imaginary(tau[1][1]));
...
function CalculThetas(tau)
  ...
  initB := NaiveThetaConstantsGenus3(taulow,1,0);
  if (flag eq 1) then
	return initB;
  else
	// we keep going
