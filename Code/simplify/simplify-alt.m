...
F0 := Evaluate(F, [x,y,((-9390)/(151*631*1733*3541*12613*64937*97187))*z]);
...
j := BinaryQuarticInvariants(S ! (f2^2 - 4*f0*f4));
...
DX := Integers() ! (2^40*I[#I]);

R0<t> := PolynomialRing(Rationals());
f0 := 2*t^5 + 7*t^4 + 2*t^3 - 9*t^2 - 2*t;

Y := HyperellipticCurve(f0);
J := IgusaInvariants(Y);
DY := Integers() ! J[#J];
n := 1;

for p in [ n : n in [1..300] | IsPrime(n) ] do
    test := true;
